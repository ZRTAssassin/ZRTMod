...
{
    public class ZRTPrismBeam : ModProjectile
    {
        // A helpful math constant for performing beam angling calculations.
        private const float PiBeamDivisor = MathHelper.Pi / ZRTPrismHoldout.NumBeams;
        ...
        public override void AI()
        {
            ...
            if (Projectile.type != ModContent.ProjectileType<ZRTPrismBeam>() || !hostPrism.active || hostPrism.type != ModContent.ProjectileType<ZRTPrismHoldout>())
            {
                Projectile.Kill();
                return;
            }
            ...
            float chargeRatio = MathHelper.Clamp(hostPrism.ai[0] / ZRTPrismHoldout.MaxCharge, 0f, 1f);
            ...
            Projectile.friendly = hostPrism.ai[0] > ZRTPrismHoldout.DamageStart;

            // This offset is used to make each individual beam orient differently based on its Beam ID.
            float beamIdOffset = BeamID - ZRTPrismHoldout.NumBeams / 2f + 0.5f;
            float beamSpread;
            float spinRate;
            ...
            float deviationAngle = (hostPrism.ai[0] + beamIdOffset * spinRate) / (spinRate * ZRTPrismHoldout.NumBeams) * MathHelper.TwoPi;
        ...
        private Color GetOuterBeamColor()
        {
            ...
            float hue = (BeamID / ZRTPrismHoldout.NumBeams) % BeamHueVariance + BeamColorHue;

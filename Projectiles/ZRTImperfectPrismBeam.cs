...
namespace ZRTMod.Projectiles
{
    public class ZRTImperfectPrismBeam : ModProjectile
    {
        ...
        private const float BeamLightBrightness = 0.70f;
        ...
        private const float BeamHueVariance = 0f; //2f;
        private const float BeamColorSaturation = 0f; //0.65f;
        private const float BeamColorLightness = 0f; //0.35f;
        ...
        public override void AI()
        {
            ...
            if (Projectile.type != ModContent.ProjectileType<ZRTImperfectPrismBeam>() || !hostPrism.active || hostPrism.type != ModContent.ProjectileType<ZRTImperfectPrismHoldout>())
            {
                Projectile.Kill();
                return;
            }
            ...
            float chargeRatio = MathHelper.Clamp(hostPrism.ai[0] / ZRTImperfectPrismHoldout.MaxCharge, 0f, 1f);
            ...
            Projectile.friendly = hostPrism.ai[0] > ZRTImperfectPrismHoldout.DamageStart;

            // This offset is used to make each individual beam orient differently based on its Beam ID.
            float beamIdOffset = BeamID - ZRTImperfectPrismHoldout.NumBeams / 2f + 0.5f;
            float beamSpread;
            float spinRate;
            ...
            float deviationAngle = (hostPrism.ai[0] + beamIdOffset * spinRate) / (spinRate * ZRTImperfectPrismHoldout.NumBeams) * MathHelper.TwoPi;
        ...
        private Color GetOuterBeamColor()
        {
            ...
            float hue = (BeamID / ZRTImperfectPrismHoldout.NumBeams) % BeamHueVariance + BeamColorHue;
        ...
        private Color GetInnerBeamColor() => Color.Green;
        ...
        {
            if (target.CanBeChasedBy(NPCID.TargetDummy))
            {
